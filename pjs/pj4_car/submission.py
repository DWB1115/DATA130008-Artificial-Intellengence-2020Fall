...
class ExactInference(object):
    ...
    def elapseTime(self):
        ...
        for ((oldTile, newTile), transProb) in list(self.transProb.items()):
            probability = self.belief.getProb(oldTile[0], oldTile[1]) * transProb
            new_belief.addProb(newTile[0], newTile[1], probability)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for tile in self.particles:
            ...
            mean = math.sqrt((agentX - X) ** 2 + (agentY - Y) ** 2)
            old = self.particles[tile]
            weight = util.pdf(mean, Const.SONAR_STD, observedDist)
            temp[tile] = old*weight

        newparticles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            tile = util.weightedRandomChoice(temp)
            newparticles[tile] += 1
